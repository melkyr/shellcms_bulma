...
# All function definitions should be loaded when dot-sourcing.
# Global variable DEFAULTS can be set here.
# Main execution logic will be wrapped further down.
...
# --- Global Variable Declarations and Defaults ---
# These will be available when dot-sourced, but their values might be
# dynamically set only when the script is run directly, or overridden by tests.
...
# Core Site Configuration (can be overridden by a future config file loader)
$Global:SiteTitle = "My PowerShell Blog"                 # Default Site Title
$Global:SiteAuthor = "PowerShell User"                   # Default Site Author
$Global:NumberOfIndexArticles = 10                     # Default number of articles on the main index page

# Site Identity & URLs (defaults, to be ideally loaded from config)
$Global:SiteUrl = "http://localhost:8000"                   # Base URL of the entire website
# $Global:CmsUrl will be set dynamically based on ContentRoot when script runs
# $Global:SiteDescription will be set dynamically or from config when script runs

# Banner Picture (relative to ContentRoot or an assets folder, TBD by template)
$Global:BannerPicture = "sit-svgrepo-com.svg"

# Button CSS Styles (defaults from Bulma, can be overridden)
$Global:CssPermanentButtonClass = "button is-primary is-light"
$Global:CssInfoButtonClass = "button is-info is-light"

# Permanent Buttons Configuration (names refer to detailed configs below)
$Global:ButtonPermanent1 = "sitehome"
$Global:ButtonPermanent2 = "contact"
$Global:ButtonPermanent3 = "" # Empty means not used

# --- Detailed Button Configurations ---
$Global:ButtonSitehomeText = "Site Home"
$Global:ButtonSitehomeUrl = $Global:SiteUrl # Will update if SiteUrl changes
$Global:ButtonSitehomeIcon = "home-svgrepo-com.svg"
$Global:ButtonSitehomeTooltip = "Main website homepage"

$Global:ButtonContactText = "Contact"
$Global:ButtonContactUrl = "$($Global:SiteUrl)/contact.html"
$Global:ButtonContactIcon = "comment-svgrepo-com.svg"
$Global:ButtonContactTooltip = "Send email to site manager"

$Global:ButtonSubhomeText = "Section Home" # Placeholder, will be updated dynamically
$Global:ButtonSubhomeUrl = "index.html"
$Global:ButtonSubhomeIcon = "book-svgrepo-com.svg"
$Global:ButtonSubhomeTooltip = "Homepage for this section"

$Global:ButtonHistoryText = "History"
$Global:ButtonHistoryUrl = "all_posts.html"
$Global:ButtonHistoryIcon = "layers-svgrepo-com.svg"
$Global:ButtonHistoryTooltip = "All posts archive"

$Global:ButtonTagsindexText = "Tags"
$Global:ButtonTagsindexUrl = "all_tags.html"
$Global:ButtonTagsindexIcon = "paper-bag-svgrepo-com.svg"
$Global:ButtonTagsindexTooltip = "All content tags"

$Global:ButtonRssText = "RSS"
$Global:ButtonRssUrl = "feed.rss"
$Global:ButtonRssIcon = "activity-svgrepo-com.svg"
$Global:ButtonRssTooltip = "Subscribe via RSS"

# Search Box Configuration
$Global:SearchBoxPages = "index post tags all_posts all_docs all_tags"
$Global:SearchBoxWidth = "40"
$Global:SearchBoxEngine = "google"

# Localization strings (example from shellcms_b)
$Global:TemplateArchiveIndexPage = "Back to the front page"

# --- Path-dependent globals are initialized by Set-GlobalPathVariables ---
# $Global:ContentRoot, $Global:TemplatesDir, $Global:HeaderTemplate, etc.
# $Global:CmsUrl (re-calculated in Set-GlobalPathVariables if needed, or after SiteUrl is finalized)
# $Global:SiteDescription (can be default here, overridden by config loader)
$Global:SiteDescription = "A static site generated by Spelunker (PowerShell)"
...
# This block will only run when the script is executed directly, not when dot-sourced.
if ($MyInvocation.MyCommand.Path -eq $PSCommandPath) {

    # Handle -Help switch immediately
    if ($PSBoundParameters.ContainsKey('Help')) {
        Show-SpelunkerHelp
        exit 0
    }

    # Set interactive mode global flag based on -Interactive switch
    ...
    # Initialize global paths based on the -Path parameter (or its default)
    # This also sets $Global:ContentRoot, $Global:TemplatesDir etc.
    Set-GlobalPathVariables -BaseContentPathFromParam $Path
    ...
    $Global:ButtonSitehomeUrl = $Global:SiteUrl # Ensure this uses the potentially updated SiteUrl
    $Global:ButtonContactUrl = "$($Global:SiteUrl)/contact.html" # Ensure this uses the potentially updated SiteUrl
    Write-Verbose "Recalculated CmsUrl: $($Global:CmsUrl)"
    Write-Verbose "Recalculated ButtonSubhomeText: $($Global:ButtonSubhomeText)"
    ...
    if ($Global:IsInteractiveMode) {
        ...
        if (-not $PSBoundParameters.ContainsKey('Path')) {
            ...
            $inputPath = Read-Host -Prompt "Enter new path for content root, or press Enter to keep current"
            if (-not [string]::IsNullOrWhiteSpace($inputPath)) {
                if (Test-Path -Path $inputPath -IsValid) {
                    Write-Verbose "User entered new path: $inputPath. Re-initializing global paths."
                    $Path = $inputPath
                    Set-GlobalPathVariables -BaseContentPathFromParam $Path # Re-initialize all paths
                    # Re-calculate CmsUrl and dynamic button texts after path change
                    if (($Global:ContentRoot | Split-Path -Leaf) -eq '.' -or ($Global:ContentRoot -eq $PSScriptRoot) ) {
                        $Global:CmsUrl = $Global:SiteUrl
                        $Global:ButtonSubhomeText = "Home"
                    } else {
                     ...
                     Write-Verbose "Recalculated CmsUrl after TUI path change: $($Global:CmsUrl)"
                     Write-Verbose "Recalculated ButtonSubhomeText after TUI path change: $($Global:ButtonSubhomeText)"
                } else {
                    Write-Warning "The path '$inputPath' seems invalid or inaccessible. Keeping current ContentRoot: $ContentRoot"
                }
            }
    ...
    switch ($Command) {
        ...
        "post" {
            if ($PSBoundParameters.ContainsKey('PostHtmRawPathParameter')) {
                ...
                if ($Global:IsInteractiveMode) {
                    $userPostPath = Read-Host -Prompt "Enter path for new/existing post .htmraw file (relative to '$($Global:ContentRoot)')"
                    if (-not [string]::IsNullOrWhiteSpace($userPostPath)) {
                        Invoke-Post -PostHtmRawPathParameter $userPostPath
                    } else {
                        Write-Warning "No post path entered. Aborting 'post' command."
                    }
                } else {
                    Write-Error "For -Command 'post', please specify -PostHtmRawPathParameter or -PostFilePath when not in interactive mode."
                }
            }
        }
        "edit" {
            ...
            } elseif ($PSBoundParameters.ContainsKey('EditFilePath')) {
                $filePathToEdit = $EditFilePath
            }
            Invoke-Edit -EditFilePathParameter $filePathToEdit
            # Invoke-Edit itself handles interactive path selection if $filePathToEdit is $null and mode is interactive
        }
        default {
            if ([string]::IsNullOrWhiteSpace($Command)) {
